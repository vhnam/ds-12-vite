...
import type { SelectOption } from "@ds-12/ui/select";
...
function StateSelect({
  ...
  if (state === "focused") {
    return <FocusedSelectWrapper>{select}</FocusedSelectWrapper>;
  }

  if (state === "opened") {
    return <OpenSelectWrapper>{select}</OpenSelectWrapper>;
  }

  return select;
}

export function SelectStatesShowcase({ size }: { size: (typeof SIZES)[number] }) {
  return (
    <div style={stateGroupStyle}>
      <div>
        <StoryCaption>enabled</StoryCaption>
        <StateSelect state="enabled" size={size} />
      </div>
      <div>
        <StoryCaption>focused</StoryCaption>
        <StateSelect state="focused" size={size} />
      </div>
      <div>
        <StoryCaption>disabled</StoryCaption>
        <StateSelect state="disabled" size={size} />
      </div>
      <div>
